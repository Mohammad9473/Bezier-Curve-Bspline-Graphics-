...
void drawBSpline() {
	...
	vec2* helperPoints = new vec2[n + 1];; // helper array to store the temporary vertices of the new iteration
	for (int i = 0; i < n + 1; i++) // at first we set it to the initial points
	...
	for (int i = p; i <= n; i++) {
		...
		for (float t = u[i]; t < u[i + 1]; t += 0.01) {
			...
			for (int j = i - p; j <= i; j++) // at first we set it to the initial points
				helperPoints[j] = points[j];

			for (int r = 1; r <= p; r++) { // generations counter

				vec2* helperPointsnew = new vec2[n + 1];

				for (int j = i - p + r; j <= i; j++) {
					a = (t - u[j]) / (u[j + p - r + 1] - u[j]);
					vec2 newPoint = // the new vertex is a combination of the vertices of the previous generation
			...
			glVertex2f(helperPoints[i].x, helperPoints[i].y); // at the last generation, the only vertex we have is the final curve point (for parameter t)
